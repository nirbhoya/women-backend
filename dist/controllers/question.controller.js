...
exports.updateMenstrualHealth = exports.createMenstrualHealth = void 0;
const MenstrualHealth_1 = __importDefault(require("../models/MenstrualHealth"));
const notification_1 = require("./notification"); // Import notification creation function
// Ensure you have this model imported
const User_1 = __importDefault(require("../models/User"));
// Assuming the notification function is in this controller
const createMenstrualHealth = async (req, res) => {
    try {
        ...
        const existingRecord = await MenstrualHealth_1.default.findOne({ userId });
        if (existingRecord) {
        ...
        const menstrualHealth = new MenstrualHealth_1.default({ userId, ...req.body });
        ...
        if (lastPeriod) {
            ...
            await (0, notification_1.createMenstrualNotification)(userId, nextDateMessage);
            ...
            if (currentDate.toDateString() === reminderDate.toDateString()) {
                ...
                await (0, notification_1.createMenstrualNotification)(userId, reminderMessage);
            }
        }
        ...
        await (0, notification_1.createMenstrualNotification)(userId, message); // Call the notification function
...
exports.createMenstrualHealth = createMenstrualHealth;
...
//   }
// };
const updateMenstrualHealth = async (req, res) => {
    try {
        const userId = req.user?.id; // Assuming authentication middleware sets req.user
        if (!userId) {
            res.status(401).json({ error: "Unauthorized" });
        ...
        const updatedData = await MenstrualHealth_1.default.findOneAndUpdate({ userId }, // Match the user's menstrual health data
